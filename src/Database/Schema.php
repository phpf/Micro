...
class Schema
{
	...
	public $table;
	// $table_basename + table prefix

	public $columns = array();
	// assoc. array of "column => SQL definition" - required.

	public $primary_key;
	// string - required.

	public $unique_keys = array();
	// array (exclusive of $primary_key)

	public $keys = array();
	// array (exclusive of $primary_key and $unique_keys)
	...
	public function isKey($column) {
		return (bool)($column === $this->primary_key || isset($this->keys[$column]) || isset($this->unique_keys[$column]));
	}
	...
	public function sqlColumnWhere($col, $val, $like_wild = false) {
		...
		if ('%s' === $format) {
			$value = \Phpf\Str::escSqlLike($value);
			if ($like_wildcard)
				$value = '%'.$value.'%';
